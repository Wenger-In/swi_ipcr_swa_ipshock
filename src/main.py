...
def main(inputDic, outputDic):
    ...
    testDic = os.path.join(inputDic, 'TestData/')
    mag_files = [f for f in os.listdir(testDic) if f.startswith('Z_MAG')]
    magFile = os.path.join(testDic, mag_files[0])
    fcup_files = [f for f in os.listdir(testDic) if f.startswith('Z_FARADAYCUP')]
    fcupFile = os.path.join(testDic, fcup_files[0])
    ...
    while True:
        ...
        if temp_data_flag == 1:  # no missing data
            ...
            ort_sess = ort.InferenceSession(os.path.join(inputDic, 'shock_checking.onnx'))
            ...
            if outputs[0][0][0] < outputs[0][0][1]:
                ...
                if last_shock_time < shock_time:
                    ...
                    savepath_png = os.path.join(outputDir, (shock_time - 30 * 60).item().strftime('%Y%m%d%H%M%S')+'.png')
...
inputDic = 'E:/Research/Program/else/swi_ipcr_swa_ipshock/input/'
outputDic = 'E:/Research/Program/else/swi_ipcr_swa_ipshock/output/TestData/'
main(inputDic, outputDic)
